...
def applist():
    """Generate manifest files list from the Steam directories and add a preset file."""
    global manifest1
    global saved_main_path
    
    # This assumes the main steamapps path from the main Steam directory
    manifest1 = os.path.join(saved_main_path, "steamapps")
    output_folder = os.path.join(saved_main_path, "applist")
    
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # Process the main Steam manifest files
    process_manifest_files(manifest1, output_folder)

    # Process additional paths
    for extra_path in saved_paths:
        manifest_extra = os.path.join(extra_path, "steamapps")
        process_manifest_files(manifest_extra, output_folder)
...
def process_manifest_files(manifest_dir, output_folder):
    """Helper function to process manifest files in a given directory."""
    if os.path.exists(manifest_dir):
        items = os.listdir(manifest_dir)
        file_counter = len(os.listdir(output_folder)) + 1  # Start numbering from existing count
