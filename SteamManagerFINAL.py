...
class SteamManagerApp:
    ...
    def animate_loading_label(self, label, container, step=0):
        dots = "." * ((step % 3) + 1)
        if container.winfo_exists():
            label.configure(text=f"Searching{dots}")
            label.after(500, lambda: self.animate_loading_label(label, container, step + 1))
    ...
    def open_folder(self, window, is_main_path=False):
        ...
        if folder_path and os.path.exists(folder_path):
            if is_main_path:
                ...
                self.save_config(main_path=self.saved_main_path, extra_paths=self.saved_paths)
                self.log(f"Additional Steam library folder added: {folder_path}")
                window.destroy()
                self.initialize_main_window()
        else:
            messagebox.showerror("Error", "Invalid folder selected.")

    def add_steam_library_folder(self):
        folder_path = filedialog.askdirectory(title="Select Additional Steam Library Folder")
        if folder_path and os.path.exists(folder_path):
            if folder_path in self.saved_paths:
                messagebox.showerror("Error", "This path has already been added.")
                return
    ...
    def initialize_main_window(self):
        ...
        ctk.CTkButton(
            self.sidebar_frame,
            text="Add Additional Steam Library Folder",
            command=self.add_steam_library_folder,
            width=200,
        ).pack(pady=2)
    ...
    def perform_search(self, query, results_frame):
        ...
        status_label = ctk.CTkLabel(loading, text="Searching")
        ...
        self.animate_loading_label(status_label, loading)
    ...
    def tutorial_window(self):
        ...
        tutorial_content = (
            ...
            "   - Add Additional Steam Library Folder: Add an extra library folder directly from the sidebar.\n\n"
            "3. Game Management:\n"
